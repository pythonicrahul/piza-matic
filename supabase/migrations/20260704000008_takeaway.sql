-- Take-away / dine-in support. An order is either 'delivery' (has a deliveries
-- row + geofence) or 'takeaway' (picked up at the store, no delivery/rider).

alter table orders
  add column if not exists fulfilment text not null default 'delivery'
  check (fulfilment in ('delivery', 'takeaway'));

-- Recreate place_order with a p_fulfilment param (drop old 12-arg signature so
-- there's no overload ambiguity). p_delivery stays null for take-away.
drop function if exists public.place_order(uuid,text,text,payment_mode,text,int,int,int,int,int,jsonb,jsonb);
...
create or replace function public.place_order(
  ...
  p_fulfilment    text  default 'delivery'
...
begin
  ...
  insert into orders (
    ...
    payment_mode, quantity_total, subtotal_paise, discount_paise, gst_paise, total_paise, fulfilment
  ) values (
    p_order_code, v_token, p_customer_id, p_name, p_phone,
    p_payment_mode, p_qty_total, p_subtotal, p_discount, p_gst, p_total, p_fulfilment
...
revoke all on function public.place_order(uuid,text,text,payment_mode,text,int,int,int,int,int,jsonb,jsonb,text) from public, anon, authenticated;
